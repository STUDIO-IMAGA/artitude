<? use IMAGA\Theme\Assets; ?>
<? use IMAGA\Theme\Navigation; ?>

<section class="element content-highlights">
  <div class="container">
    <div class="row">
      <div class="col-12 text-center">
        <div class="title h1">
          Artitude. Met het puntje op de i.
        </div>
      </div>
    </div>
    <div class="row justify-content-center">
      <div class="col-12 col-md-8 col-lg-6 text-center">
        <div class="text">
          <p>Naast concept, decorbouw en beleving verzrogen wij ook de details. <br/>Licht, geluid, signing en bijvoorbeeld ict. </p>
        </div>
      </div>
    </div>
    <div class="row tiles">

      <div class="col-12 col-md-6 col-lg">
        <a href="#">
          <div class="tile">
            <div class="content">
              <div class="icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50">
                  <path d="M34.62,47.86H15.38a.42.42,0,0,1-.42-.42V17.52a.43.43,0,0,1,.42-.43H34.62a.43.43,0,0,1,.42.43V47.44A.42.42,0,0,1,34.62,47.86ZM15.81,47H34.19V18H15.81Z"/>
                  <path d="M9,47.86H.43A.42.42,0,0,1,0,47.44V2.56a.42.42,0,0,1,.43-.42H9a.42.42,0,0,1,.43.42V47.44A.42.42,0,0,1,9,47.86ZM.86,47H8.55V3H.86Z"/>
                  <path d="M49.57,47.86H41a.42.42,0,0,1-.43-.42V2.56A.42.42,0,0,1,41,2.14h8.54a.42.42,0,0,1,.43.42V47.44A.42.42,0,0,1,49.57,47.86ZM41.45,47h7.69V3H41.45Z"/>
                  <rect x="41.03" y="27.78" width="8.55" height="0.86"/>
                  <rect x="0.43" y="27.78" width="8.55" height="0.86"/>
                  <path d="M25,43.59a5.77,5.77,0,1,1,5.77-5.77A5.78,5.78,0,0,1,25,43.59Zm0-10.68a4.91,4.91,0,1,0,4.91,4.91A4.91,4.91,0,0,0,25,32.91Z"/>
                  <path d="M30.34,21.58a.22.22,0,1,1,0,.43.21.21,0,0,1-.21-.22.21.21,0,0,1,.21-.21m0-.85a1.07,1.07,0,1,0,1.07,1.06,1.07,1.07,0,0,0-1.07-1.06Z"/>
                  <path d="M19.66,21.58a.21.21,0,0,1,.21.21.21.21,0,0,1-.21.22.22.22,0,1,1,0-.43m0-.85a1.07,1.07,0,1,0,1.07,1.06,1.07,1.07,0,0,0-1.07-1.06Z"/>
                  <path d="M25,27.56A2.56,2.56,0,1,1,27.56,25,2.56,2.56,0,0,1,25,27.56Zm0-4.27A1.71,1.71,0,1,0,26.71,25,1.72,1.72,0,0,0,25,23.29Z"/>
                </svg>
              </div>
              <div class="title">
                Audio
              </div>
              <div class="text">
                <p>Wij zijn er voor de vorm en inhoudelijke technische aspecten. Vanaf het begin van je idee bn. Vanaf het begin van je idee betrokken tot een gedegen en geslaagd eindresultaat. <b>Meer</b></p>
              </div>
            </div>
          </div>
        </a>
      </div>

      <div class="col-12 col-md-6 col-lg">
        <a href="#">
          <div class="tile">
            <div class="content">
              <div class="icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50">
                  <rect x="24.57" y="4.7" width="0.86" height="8.55"/>
                  <path d="M33.55,41.45H16.45A.42.42,0,0,1,16,41V17.52a.43.43,0,0,1,.42-.43h17.1a.43.43,0,0,1,.42.43V41A.42.42,0,0,1,33.55,41.45ZM16.88,40.6H33.12V18H16.88Z"/>
                  <path d="M29.27,18H20.73a.43.43,0,0,1-.43-.43V13.25a.43.43,0,0,1,.43-.43h8.54a.43.43,0,0,1,.43.43v4.27A.43.43,0,0,1,29.27,18Zm-8.12-.86h7.7V13.68h-7.7Z"/>
                  <path d="M31.41,5.13H18.59a4.71,4.71,0,0,1-4.7-4.7A.43.43,0,0,1,14.32,0H35.68a.43.43,0,0,1,.43.43A4.71,4.71,0,0,1,31.41,5.13ZM14.77.86a3.84,3.84,0,0,0,3.82,3.41H31.41A3.84,3.84,0,0,0,35.23.86Z"/>
                  <path d="M25,50a.43.43,0,0,1-.43-.43V47.44a.43.43,0,0,1,.86,0v2.13A.43.43,0,0,1,25,50Z"/>
                  <path d="M32.55,46.87a.42.42,0,0,1-.3-.12l-1.51-1.51a.43.43,0,0,1,.61-.61l1.51,1.51a.45.45,0,0,1,0,.61A.47.47,0,0,1,32.55,46.87Z"/>
                  <path d="M17.45,46.87a.47.47,0,0,1-.31-.12.45.45,0,0,1,0-.61l1.51-1.51a.43.43,0,0,1,.61.61l-1.51,1.51A.42.42,0,0,1,17.45,46.87Z"/>
                </svg>
              </div>
              <div class="title">
                Licht
              </div>
              <div class="text">
                <p>Wij zijn er voor de vorm en inhoudelijke technische aspecten. Vanaf het begin van je idee bn. Vanaf het begin van je idee betrokken tot een gedegen en geslaagd eindresultaat. <b>Meer</b></p>
              </div>
            </div>
          </div>
        </a>
      </div>

      <div class="col-12 col-md-6 col-lg">
        <a href="#">
          <div class="tile">
            <div class="content">
              <div class="icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50">
                  <path d="M45.3,50H4.7A4.71,4.71,0,0,1,0,45.3V4.7A4.71,4.71,0,0,1,4.7,0H45.3A4.71,4.71,0,0,1,50,4.7V45.3A4.71,4.71,0,0,1,45.3,50ZM4.7.86A3.84,3.84,0,0,0,.86,4.7V45.3A3.84,3.84,0,0,0,4.7,49.14H45.3a3.84,3.84,0,0,0,3.84-3.84V4.7A3.84,3.84,0,0,0,45.3.86Z"/>
                  <path d="M49.57,11.54H.43A.43.43,0,0,1,0,11.11a.43.43,0,0,1,.43-.43H49.57a.43.43,0,0,1,.43.43A.43.43,0,0,1,49.57,11.54Z"/>
                  <path d="M7.91,7.27A1.5,1.5,0,1,1,9.4,5.77,1.5,1.5,0,0,1,7.91,7.27Zm0-2.14a.64.64,0,0,0,0,1.28.64.64,0,0,0,0-1.28Z"/>
                  <path d="M14.32,7.27a1.5,1.5,0,1,1,1.49-1.5A1.5,1.5,0,0,1,14.32,7.27Zm0-2.14a.64.64,0,1,0,0,1.28.64.64,0,1,0,0-1.28Z"/>
                  <path d="M20.73,7.27a1.5,1.5,0,1,1,1.49-1.5A1.5,1.5,0,0,1,20.73,7.27Zm0-2.14a.64.64,0,1,0,0,1.28.64.64,0,1,0,0-1.28Z"/>
                  <path d="M13.25,43.59H6.84a.43.43,0,0,1-.43-.43V17.52a.43.43,0,0,1,.43-.43h6.41a.44.44,0,0,1,.43.43V43.16A.43.43,0,0,1,13.25,43.59Zm-6-.86h5.55V18H7.27Z"/>
                  <path d="M13.25,24.36H6.84a.43.43,0,0,1-.43-.43.43.43,0,0,1,.43-.43h6.41a.44.44,0,0,1,.43.43A.43.43,0,0,1,13.25,24.36Z"/>
                  <path d="M13.25,30.77H6.84a.43.43,0,0,1-.43-.43.43.43,0,0,1,.43-.43h6.41a.44.44,0,0,1,.43.43A.43.43,0,0,1,13.25,30.77Z"/>
                  <path d="M13.25,37.18H6.84a.43.43,0,0,1-.43-.43.43.43,0,0,1,.43-.43h6.41a.44.44,0,0,1,.43.43A.43.43,0,0,1,13.25,37.18Z"/>
                  <path d="M43.16,43.59H30.34a.43.43,0,0,1-.43-.43V32.48a.43.43,0,0,1,.43-.43h3.21a.43.43,0,0,1,.27.1l2.42,2h6.92a.43.43,0,0,1,.43.43v8.54A.43.43,0,0,1,43.16,43.59Zm-12.39-.86h12V35H36.08a.39.39,0,0,1-.27-.1l-2.42-2H30.77Z"/>
                  <path d="M22.65,29.85a.43.43,0,0,1-.21-.8l15.44-8.73a.43.43,0,0,1,.58.16.44.44,0,0,1-.16.59L22.87,29.8A.54.54,0,0,1,22.65,29.85Z"/>
                  <path d="M40,22.22a2.44,2.44,0,0,1-.71-.1,2.56,2.56,0,0,1-.53-4.71,2.6,2.6,0,0,1,2-.22,2.58,2.58,0,0,1,1.75,3.18A2.52,2.52,0,0,1,41.2,21.9,2.6,2.6,0,0,1,40,22.22ZM40,18a1.71,1.71,0,0,0-1.64,1.23,1.76,1.76,0,0,0,.15,1.31,1.72,1.72,0,0,0,1,.81,1.69,1.69,0,0,0,1.3-.15,1.7,1.7,0,0,0,.82-1,1.69,1.69,0,0,0-.15-1.3h0a1.65,1.65,0,0,0-1-.81A1.59,1.59,0,0,0,40,18Z"/>
                  <path d="M20.73,32.91a2.56,2.56,0,1,1,1.1-.25h0A2.67,2.67,0,0,1,20.73,32.91Zm.91-.64h0Zm-.92-3.64a1.77,1.77,0,0,0-.73.17,1.72,1.72,0,0,0-.81,2.28,1.7,1.7,0,0,0,1,.87,1.66,1.66,0,0,0,1.31-.06,1.71,1.71,0,0,0-.74-3.26Z"/>
                </svg>
              </div>
              <div class="title">
                Grafisch
              </div>
              <div class="text">
                <p>Wij zijn er voor de vorm en inhoudelijke technische aspecten. Vanaf het begin van je idee bn. Vanaf het begin van je idee betrokken tot een gedegen en geslaagd eindresultaat. <b>Meer</b></p>
              </div>
            </div>
          </div>
        </a>
      </div>

      <div class="col-12 col-md-6 col-lg">
        <a href="#">
          <div class="tile">
            <div class="content">
              <div class="icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50">
                  <rect x="44.87" y="8.97" width="0.86" height="32.05"/>
                  <rect x="8.97" y="4.27" width="32.05" height="0.86"/>
                  <path d="M41,45.73H4.7a.44.44,0,0,1-.43-.43V9h.86v35.9H41Z"/>
                  <path d="M9,9.4H.43A.43.43,0,0,1,0,9V.43A.43.43,0,0,1,.43,0H9A.43.43,0,0,1,9.4.43V9A.43.43,0,0,1,9,9.4ZM.86,8.55H8.55V.86H.86Z"/>
                  <path d="M49.57,9.4H41A.43.43,0,0,1,40.6,9V.43A.43.43,0,0,1,41,0h8.54A.43.43,0,0,1,50,.43V9A.43.43,0,0,1,49.57,9.4Zm-8.12-.85h7.69V.86H41.45Z"/>
                  <path d="M49.57,50H41a.43.43,0,0,1-.43-.43V41A.43.43,0,0,1,41,40.6h8.54A.43.43,0,0,1,50,41v8.54A.43.43,0,0,1,49.57,50Zm-8.12-.86h7.69V41.45H41.45Z"/>
                  <path d="M21.79,45.73H4.7a.44.44,0,0,1-.43-.43V28.21a.43.43,0,0,1,.43-.43H21.79a.43.43,0,0,1,.43.43V45.3A.43.43,0,0,1,21.79,45.73ZM5.13,44.87H21.37V28.63H5.13Z"/>
                  <path d="M21.79,28.63a.46.46,0,0,1-.3-.12.45.45,0,0,1,0-.61L32.18,17.22a.42.42,0,1,1,.6.6L22.1,28.51A.47.47,0,0,1,21.79,28.63Z"/>
                  <path d="M32.48,26.5a.43.43,0,0,1-.43-.43V18H23.93a.43.43,0,0,1-.43-.43.44.44,0,0,1,.43-.43h8.55a.44.44,0,0,1,.43.43v8.55A.44.44,0,0,1,32.48,26.5Z"/>
                </svg>
              </div>
              <div class="title">
                Signing
              </div>
              <div class="text">
                <p>Wij zijn er voor de vorm en inhoudelijke technische aspecten. Vanaf het begin van je idee bn. Vanaf het begin van je idee betrokken tot een gedegen en geslaagd eindresultaat. <b>Meer</b></p>
              </div>
            </div>
          </div>
        </a>
      </div>

      <div class="col-12 col-md-6 col-lg">
        <a href="#">
          <div class="tile">
            <div class="content">
              <div class="icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50">
                  <path d="M47,45.38H31a.47.47,0,0,1-.41-.23l-5.37-8.82-5.3,8.82a.47.47,0,0,1-.4.23H3.34a.48.48,0,0,1-.48-.47V24a.49.49,0,0,1,.48-.48H47a.48.48,0,0,1,.47.48v20.9A.47.47,0,0,1,47,45.38Zm-15.81-1H46.56V24.48H3.81v20H19.22l5.56-9.27a.49.49,0,0,1,.41-.23h0a.47.47,0,0,1,.4.23Z"/>
                  <path d="M3.34,24.48a.45.45,0,0,1-.25-.07.48.48,0,0,1-.16-.65L13.58,6.23a3.26,3.26,0,0,1,2-1.51,3.32,3.32,0,0,1,2.52.37.48.48,0,0,1,.16.65.48.48,0,0,1-.65.17,2.37,2.37,0,0,0-3.26.81L3.75,24.25A.48.48,0,0,1,3.34,24.48Z"/>
                  <path d="M47,24.48a.48.48,0,0,1-.41-.23L36,6.72a2.38,2.38,0,0,0-3.26-.81.49.49,0,0,1-.66-.17.48.48,0,0,1,.17-.65A3.33,3.33,0,0,1,36.8,6.23L47.44,23.76a.46.46,0,0,1-.16.65A.42.42,0,0,1,47,24.48Z"/>
                  <path d="M36.66,39.94a.57.57,0,0,1-.18,0,.49.49,0,0,1-.3-.44V28.4a.49.49,0,0,1,.3-.44.46.46,0,0,1,.51.11l5.54,5.53a.48.48,0,0,1,0,.67L37,39.8A.47.47,0,0,1,36.66,39.94Zm.47-10.39v8.77l4.39-4.38Z"/>
                  <path d="M7.52,30.7a1.81,1.81,0,0,1-.43-3.56,1.76,1.76,0,0,1,1.36.2,1.8,1.8,0,0,1,.62,2.48A1.79,1.79,0,0,1,8,30.65,2.2,2.2,0,0,1,7.52,30.7Zm0-2.67a.64.64,0,0,0-.2,0,.86.86,0,0,0-.63,1,.85.85,0,0,0,1,.62.84.84,0,0,0,.62-1h0A.81.81,0,0,0,8,28.16.84.84,0,0,0,7.52,28Z"/>
                  <path d="M12,30.7a1.79,1.79,0,0,1-.93-.26A1.82,1.82,0,0,1,10.43,28a1.81,1.81,0,0,1,3.31.5h0a1.83,1.83,0,0,1-1.32,2.19A2.28,2.28,0,0,1,12,30.7ZM12,28a.76.76,0,0,0-.21,0,.82.82,0,0,0-.52.39.86.86,0,0,0,.29,1.18.82.82,0,0,0,.65.09.84.84,0,0,0,.62-1h0a.81.81,0,0,0-.39-.53A.84.84,0,0,0,12,28Z"/>
                  <path d="M16.25,30.7a1.81,1.81,0,0,1-.43-3.56,1.81,1.81,0,0,1,2,2.68,1.79,1.79,0,0,1-1.11.83A2.2,2.2,0,0,1,16.25,30.7Zm0-2.67a.76.76,0,0,0-.21,0,.86.86,0,0,0-.62,1,.85.85,0,0,0,1,.62.84.84,0,0,0,.62-1h0a.81.81,0,0,0-.39-.53A.84.84,0,0,0,16.25,28Z"/>
                  <path d="M28.13,30.67H22.25A1.27,1.27,0,0,1,21,29.4V28a1.27,1.27,0,0,1,1.27-1.27h5.88A1.27,1.27,0,0,1,29.4,28V29.4A1.27,1.27,0,0,1,28.13,30.67Zm-5.88-3a.33.33,0,0,0-.32.32V29.4a.32.32,0,0,0,.32.31h5.88a.32.32,0,0,0,.32-.31V28a.33.33,0,0,0-.32-.32Z"/>
                  <path d="M3.14,33.17C1.39,33.17,0,32.11,0,30.79V27.14c0-1.32,1.42-2.38,3.17-2.38v.8c-1.28,0-2.37.72-2.37,1.58v3.65c0,.86,1.09,1.58,2.37,1.58Z"/>
                  <path d="M46.86,33.17v-.8c1.28,0,2.37-.72,2.37-1.58V27.14c0-.86-1.09-1.58-2.37-1.58v-.8c1.75,0,3.17,1.06,3.17,2.38v3.65C50,32.11,48.61,33.17,46.86,33.17Z"/>
                  <path d="M32.53,24h-1c0-1.26-1.34-2.29-3-2.29h-6.4c-1.65,0-3,1-3,2.29h-.95c0-1.79,1.76-3.24,3.94-3.24h6.4C30.76,20.77,32.53,22.22,32.53,24Z"/>
                </svg>
              </div>
              <div class="title">
                ICT
              </div>
              <div class="text">
                <p>Wij zijn er voor de vorm en inhoudelijke technische aspecten. Vanaf het begin van je idee bn. Vanaf het begin van je idee betrokken tot een gedegen en geslaagd eindresultaat. <b>Meer</b></p>
              </div>
            </div>
          </div>
        </a>
      </div>

    </div>
  </div>
</section>

<section class="element footer-front-page p-3 p-sm-0">
  <div class="container border border-white">
    <div class="row border-bottom border-white">
      <div class="col-12 col-md-4 border-right border-white p-3 p-md-5">
        <div class="title">
          <div>
            <span class="h4">Deze klanten hadden een idee..</span>
          </div>
          <div class="text-left text-md-right">
            <span class="h4">waar wij realiteit van maakten</span>
          </div>
        </div>
      </div>
      <div class="col-12 col-md-8">
        <div class="customers">
          <? $klanten = get_field('customers','options'); ?>
          <? if($klanten): ?>
            <? foreach($klanten as $klant): ?>
              <div class="customers-item">
                <img class="img-fluid" src="<?= $klant['image']['url'] ?>" alt="<?= $klant["customer_name"]?>"/>
              </div>
            <? endforeach; ?>
          <? endif; ?>
        </div>
      </div>
    </div>
    <div class="row align-items-center">
      <div class="col-12 col-md-4 pr-0 py-md-6 p-3 pl-md-5">
        <h6>Een keertje vrijblijvend sparren over je idee?</h6>
      </div>
      <div class="col-12 col-md-8 pl-0">
        <?= Navigation\navigation( 'secondary_navigation', 'footer_front_page_navigation' , 0, 1, "nav contact-menu light", ""); ?>
      </div>
    </div>
  </div>
</section>
